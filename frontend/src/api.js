...
export async function bulkImportProductsApi(products) {
  try {
    ...
    if (!res.ok) throw new Error(`Bulk import failed with status ${res.status}`);
    return await res.json();
  } catch (err) {
...
export async function submitStockTakeApi(items, notes = '') {
  try {
    const res = await fetch(`${API_BASE}/stock-take/submit`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ items, notes })
    });
    if (!res.ok) throw new Error(`Stock take submission failed with status ${res.status}`);
    return await res.json();
  } catch (err) {
    console.warn('Backend stock take submit error:', err);
    return null;
  }
}
